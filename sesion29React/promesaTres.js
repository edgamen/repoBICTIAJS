//codigo de Adrian Tibocha para analizar
function empezarDia(hora) {
    return promise = new Promise((resolved, reject) => {
        setTimeout(() => {
            return (!(hora == "06:00am")) ? reject(new Error(`no suena el despertador`)) : resolved(true)
        })
    })
}
...
function tomarTransporte(hora) {
    ...
    return promise = new Promise((resolved, reject) => {
        setTimeout(() => {
            return (!(hora == "07:00am")) ? reject(new Error(`paro en el portal y no puede coger transporte
            `)) : resolved(true)
        }, 2000)
    })
}
...
function estudiar(hora) {
    ...
    return promise = new Promise((resolved, reject) => {
        setTimeout(() => {
            return (!(hora == "08:20am")) ? reject(new Error(`llego tarde y no puede entrar a clases`)) : resolved(true)
        }, 2000)
    })
}
...
function volverCasa(hora) {
    ...
    return promise = new Promise((resolved, reject) => {
        setTimeout(() => {
            return (!(hora == "11:05pm")) ? reject(new Error(`ha perdido las llaves de su casa y le toca dormir en 
